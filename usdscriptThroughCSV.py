"""
Functional script version with CSV Files
"""
...
if __name__ == "__main__":
    ...
    try:
        # Create a DataFrame from the toCSVFile list
        ...
        if not file_exists:
            ...
            print(f"XLSX file '{XLSX_FILENAME}' created with headers and data.")
        else:
            ...
            except FileNotFoundError: # Should not happen if file_exists is true, but as a safeguard
                df_new_row.to_excel(XLSX_FILENAME, index=False, header=True, engine='openpyxl')
                print(f"XLSX file '{XLSX_FILENAME}' was not found despite check, created new.")

            print(f"Data appended to existing XLSX file '{XLSX_FILENAME}'.")

    except Exception as e:
        print(f"An error occurred during XLSX writing with pandas: {e}")
