...
def getHourAlCambio(api_data):
    if api_data and api_data.get("last_update"):
        try:
            ...
            return api_data.get("last_update").split(",")[1].strip()
        except (IndexError, AttributeError, TypeError) as e:
            print(f"Warning: Error parsing hour from last_update '{api_data.get('last_update', 'N/A')}': {e}")
            return "N/A"
    print("Warning: Could not extract hour (api_data was None or last_update missing).")
    return "N/A"
...
def exchangesPrice(api_data):
    if api_data and api_data.get("price") is not None:
        ...
        if isinstance(price, (int, float)):
            ...
            return "N/A"
    print("Warning: Could not extract price (api_data was None or price missing).")
    return "N/A"


def getAveragePrice(bcv, par):
    if isinstance(bcv, (int, float)) and isinstance(par, (int, float)):
        return round((bcv + par) / 2, 2)
    # Opcional: Advertencia más específica si se esperaban números pero son "N/A"
    # if bcv != "N/A" or par != "N/A": 
    #     print(f"Warning: Cannot calculate average for non-numeric inputs: BCV='{bcv}', PARALELO='{par}'")
    return "N/A"
...
if __name__ == "__main__":
    ...
    formatted_date_for_excel = "N/A"
    hour_for_excel = "N/A"
    bcv_price_for_excel = "N/A"
    par_price_for_excel = "N/A"
    avg_price_for_excel = "N/A"
    ...
    if data_from_api:
        formatted_date_for_excel = get_formatted_date_from_api(data_from_api)
        sources_data = data_from_api.get("monitors")
        alcambio_data = None
        bcv_data = None
        ...
        if sources_data:
            ...
            print("Warning: 'monitors' key not found in API response. Exchange data will be N/A.")

        hour_for_excel = getHourAlCambio(alcambio_data) 
        bcv_price_for_excel = exchangesPrice(bcv_data)   
        par_price_for_excel = exchangesPrice(alcambio_data) 
        avg_price_for_excel = getAveragePrice(bcv_price_for_excel, par_price_for_excel)
    
    else: # data_from_api is None
        print("\nFailed to retrieve data from the API. Excel row will contain placeholders.")
        formatted_date_for_excel = get_formatted_date_from_api(None)
